...
#[tokio::test]
async fn actual_sbf_controller_remains_upgradeable_by_three_seats_after_450_slots() {
    ...
    assert_eq!(before.upgrade_authority, Some(authority));
    let upgrade = Upgrade {
        buffer,
    ...
    let mut data = [0; 192];
    data[..184].copy_from_slice(&upgrade.try_to_vec().unwrap());
    let create = ix(
        program,
        vec![
            AccountMeta::new(ctx.payer.pubkey(), true),
    ...
    let execute = ix(
        ...
        GovInstruction::ExecuteControllerUpgrade { digest },
    );
    assert!(
    ...
    assert_eq!(after.upgrade_authority, Some(authority));
